...
- **PowerShell completion is now installable, opt-in (`win/nova.nsi` components page).** PowerShell
  has no auto-load directory for argument completers, so a `$PROFILE` line is the only mechanism --
  which is why nothing wired it up before. New `nova-profile.ps1` (generated by `win/dist.sh`, so
  the zip has it too) adds or `-Remove`s that line; the installer offers it as an off-by-default
  component and the uninstaller runs it with `-Remove`. **PowerShell edits its own profile rather
  than NSIS doing it**: NSIS would have to guess the file's encoding, and guessing wrong corrupts a
  file the user owns. Idempotent both ways. The MSI has no components page, so it just ships the
  script. Caveat stated in the component description: on a per-machine install this is the profile
  of whoever runs the installer, not of every user.
  Verified here before shipping: the script against a fake profile under pwsh 7.6.6 (adds the line,
