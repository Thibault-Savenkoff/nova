/* yaifdec x.yaif out.raw [preview]: RGBA of every frame (or of the PREV), concatenated, as
   test/js_dump.js does for the JavaScript decoder (test/libyaif.sh compares it with yaif). */
#include "yaifdec.h"
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
int main(int argc, char **argv) {
  ...
  int w, h, frames = 1;
  clock_t t0 = clock();
  if (argc < 3) { fprintf(stderr, "usage: yaifdec x.yaif out.raw [preview]\n"); return 2; }
  f = fopen(argv[1], "rb");
