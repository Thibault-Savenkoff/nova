// node test/js_workers.js x.yaif out.raw: decodes through the viewer's Web Worker runner (docs/index.html),
// emulated on worker_threads. Run from yaif-lisaac/; compare out.raw with yaif decode (see test/js.sh).
const fs = require('fs'), path = require('path'), { Worker: W } = require('worker_threads');
const web = path.resolve('docs');
global.YaifDecode = require(web + '/yaif_decode.js');
...
global.Worker = class {
  constructor(f) {
    ...
    const code = `const {parentPort}=require('worker_threads');global.self=global;self.location={search:''};global.importScripts=f=>{Object.assign(global,{YaifDecode:require(${JSON.stringify(web)}+'/'+f)})};
...
(async () => {
  ...
  let last = 0;
  const r = await YaifDecode.decode(d, { run: runner(x => { if (x - last > 0.2 || x === 1) { process.stdout.write((x * 100).toFixed(0) + '% '); last = x; } }) });
