...
s, ls = os.path.getsize('$T/t.jpg'), len(buf.getvalue()); p, lp = psnr(n), psnr(Image.open(buf))
...
print(('ok' if ok else 'bad') + ' %d bytes %.2f dB (libjpeg %d bytes %.2f dB)' % (s, p, ls, lp))" 2>&1)
