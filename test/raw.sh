#!/bin/bash
# RAW (level 6): each file of $@ (default ~/photos-yaif/cr3/*.CR3) goes to .yaif then to DNG; LibRaw
# must read the same sensor frame and the same parameters (levels, colour, white balance, EXIF) from
# the DNG as from the source. Prints YAIF size vs the source file, encode and decode times.
# Run from yaif-lisaac/: test/raw.sh [files...]
cd "$(dirname "$0")/.."
export LC_ALL=C
T=$(mktemp -d)
trap 'rm -rf "$T"' EXIT
gcc -O2 -o "$T/rawdump" test/rawdump.c -ldl || exit 1
[ $# -eq 0 ] && set -- ~/photos-yaif/cr3/*.CR3
fail=0
for f in "$@"; do
  t0=$(date +%s.%N)
  ./yaif encode "$f" "$T/r.yaif" > /dev/null || { echo "FAIL $f: encode"; fail=1; continue; }
  t1=$(date +%s.%N)
  ./yaif decode "$T/r.yaif" "$T/r.dng" > /dev/null || { echo "FAIL $f: decode"; fail=1; continue; }
  t2=$(date +%s.%N)
  "$T/rawdump" "$f" "$T/ref.bin" > "$T/ref.txt"
  "$T/rawdump" "$T/r.dng" "$T/dec.bin" > "$T/dec.txt"
  s=$(stat -c %s "$f"); n=$(stat -c %s "$T/r.yaif")
...
f=$1
case "$f" in *.CR3|*.cr3)
  ./yaif encode "$f" "$T/r.yaif" > /dev/null
  python3 - "$f" "$T/r.yaif" <<'PY' && echo "OK   $(basename "$f") CMT1-4 kept" || { echo "FAIL $(basename "$f") metadata"; fail=1; }
import sys, struct
...
if command -v darktable-cli > /dev/null; then
  ./yaif decode "$T/r.yaif" "$T/b.dng" > /dev/null
  ext=${f##*.}
  cp "$f" "$T/a.$ext"
...
# Development (yaif decode to .tif/.png): the same image as LibRaw's on the source file (test/develop.c,
...
./yaif encode "$f" "$T/r.yaif" > /dev/null
./yaif decode "$T/r.yaif" "$T/d.tif" > /dev/null && ./yaif decode "$T/r.yaif" "$T/d.png" > /dev/null && ./yaif preview "$T/r.yaif" "$T/p.png" > /dev/null \
...
./yaif decode "$T/r.yaif" "$T/k.tif" -look darktable > /dev/null && "$T/develop" "$f" "$T/refk.ppm" 1 && uv run -q --with numpy --with tifffile python -c "
