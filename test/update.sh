#!/bin/bash
# The update check (yaif_update.li): once a day, in a terminal only, never blocking.
# A fake curl stands in for GitHub (no network), and script(1) gives yaif a terminal.
# Run from yaif-lisaac/ after building ./yaif: test/update.sh
set -eu
cd "$(dirname "$0")/.."
[ -x ./yaif ] || { echo "build ./yaif first (lisaac yaif.li -boost)"; exit 1; }
...
EOF
chmod +x "$T/bin/curl"
cache="$T/cache/yaif/releases.json"

# run [env...]: ./yaif --version in a terminal, with its stderr (the notice) in $T/out
run() {
  env -i PATH="$T/bin:/usr/bin:/bin" HOME="$T/home" XDG_CACHE_HOME="$T/cache" "$@" \
    script -qec "$PWD/yaif --version; sleep 0.5" /dev/null > "$T/out" 2>&1
...
env -i PATH="$T/bin:/usr/bin:/bin" HOME="$T/home" XDG_CACHE_HOME="$T/cache" ./yaif --version > "$T/out" 2>&1
sleep 0.3
...
run
check "same day: notice" grep -q 'yaif 2.1.0 is available (this is 2.0.0-beta)' "$T/out"
check "same day: no refresh" [ "$(calls)" = 1 ]
check "same day: stdout untouched" grep -q '^yaif 2.0.0-beta' "$T/out"

# --- turned off ---
run YAIF_NO_UPDATE_CHECK=1
check "YAIF_NO_UPDATE_CHECK=1: no notice" no available "$T/out"
...
run
check "2.0.0 > 2.0.0-beta, other major ignored" grep -q 'yaif 2.0.0 is available' "$T/out"
