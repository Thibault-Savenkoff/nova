/* Windows build of yaif (MinGW-w64): the POSIX calls of yaif.c mapped to Win32. Force-included by
   win/build.sh; win/sys/*.h and win/dlfcn.h are empty so the POSIX #includes still resolve.
   No fork: yaif_par.li runs parallel jobs in replicas (copies of the process started with
...
/* Replicas (yaif_par.li) run the same command: only replica 0 (no YAIF_REPLICA, or "0/N") writes files. */
static int yaif_replica(void) { const char *r = getenv("YAIF_REPLICA"); return r && atoi(r) > 0; }
static FILE *yaif_fopen(const char *name, const char *mode) {
  return fopen(yaif_replica() && strpbrk(mode, "wa+") ? "NUL" : name, mode);
}
#define fopen yaif_fopen

/* Progress line: UTF-8 bar and "\033[K" (erase line) need these console modes. */
__attribute__((constructor)) static void yaif_console(void) {
...
/* mmap: unused (yaif_par.li allocates with VirtualAlloc on Windows), kept for the POSIX code paths */
#define PROT_READ 1
#define PROT_WRITE 2
#define MAP_SHARED 1
