/* WebAssembly build: nova loads LibRaw and zlib with dlopen/dlsym; here they are linked in, and
   build.sh renames nova.c's dlopen/dlsym to these. Other libraries (libheif, libwebp) stay "not found". */
#include <string.h>
#include <zlib.h>
#include "../third_party/libraw/libraw/libraw.h"

/* nova declares adler32_combine with a long length; this zlib.h maps it to the 64-bit variant, and a
...
void *nova_dlopen(const char *name, int flags) {
  (void)flags;
  return name && (!strncmp(name, "libraw_r.so", 11) || !strncmp(name, "libz.so", 7)) ? (void *)syms : 0;
}

void *nova_dlsym(void *h, const char *s) {
  unsigned i;
  if (!h || !s) return 0;
