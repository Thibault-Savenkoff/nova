// YAIF v2 decoder in JavaScript: port of the decoding paths of yaif_codec.li, yaif_model.li,
// yaif_lossy.li, yaif_wavelet.li and the container of yaif.li. Must give the same pixels as
// `yaif decode` bit for bit (test/js.sh). Lisaac Int is int64: every product that can pass 2^31
...
// Signed int32 result: the same 32 bits as the unsigned hash of yaif_model.li (group() takes it mod 2^32).
...
// ---------------------------------------------------------------------------------------------
// Region decoder (yaif_codec.li decode). Runs the stripes through `run` (sequential by default;
...
function parse(data) {
  if (data.length < 9 || u32(data, 0) !== 0x89594149 || u32(data, 4) !== 0x460D0A1A || data[8] !== 0x0A) throw new Error('not a YAIF file');
  ...
  while (pos < data.length) {
    ...
    if (t === T.IHDR) {
      ...
      if (data[p + 10] !== 2) throw new Error('unsupported YAIF version (only v2)');
...
// headroomBase, headroomAlt} in stops, or null. As yaif_hdr.li.
...
// the gain map's ISO metadata: hdrgm XMP in each, MPF index after the primary's APP0. As yaif_hdr.li.
...
if (typeof self !== 'undefined') self.YaifDecode = api;
