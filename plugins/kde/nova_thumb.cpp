...
// image/x-nova needs this one. The decoding is the Qt plugin's (plugins/qt), through QImageReader.
#include <KIO/ThumbnailCreator>
#include <KPluginFactory>
#include <QImage>
#include <QImageReader>

class NovaThumbnail : public KIO::ThumbnailCreator {
  Q_OBJECT
public:
  NovaThumbnail(QObject *parent, const QVariantList &args) : KIO::ThumbnailCreator(parent, args) {}

  KIO::ThumbnailResult create(const KIO::ThumbnailRequest &request) override {
    QImageReader r(request.url().toLocalFile(), "nova");
...
K_PLUGIN_CLASS_WITH_JSON(NovaThumbnail, "nova_thumb.json")

#include "nova_thumb.moc"
