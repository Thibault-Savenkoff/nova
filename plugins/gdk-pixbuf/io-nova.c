/* gdk-pixbuf loader for NOVA (.nova), on libnovadec: Eye of GNOME, GIMP (import), GTK apps and
...
static GdkPixbuf *decode(const guint8 *d, gsize n, NovaContext *ctx, GError **error) {
  nova_info info;
  uint8_t *px;
  int w, h, alpha, sw, sh;
  GdkPixbuf *pb;
  char o[2] = { 0, 0 };
  if (nova_read_info(d, n, &info)) {
    g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_CORRUPT_IMAGE, "NOVA: %s", info.error);
    return NULL;
  }
  ...
  if (info.raw) px = nova_decode_preview(d, n, &w, &h);
  else px = nova_decode(d, n, &info);
  if (!px) {
    g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_CORRUPT_IMAGE, "NOVA: %s", info.raw ? "no preview" : info.error);
    return NULL;
  }
  alpha = info.planes == 4;
  ...
  if (!pb) { free(px); g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_INSUFFICIENT_MEMORY, "NOVA: out of memory"); return NULL; }
...
static GdkPixbuf *nova_load(FILE *f, GError **error) {
...
static gpointer begin_load(GdkPixbufModuleSizeFunc size_func, GdkPixbufModulePreparedFunc prepared_func,
  ...
  NovaContext *ctx = g_new0(NovaContext, 1);
  (void)error;
...
G_MODULE_EXPORT void fill_info(GdkPixbufFormat *info) {
  static GdkPixbufModulePattern signature[] = { { "\x89NOVA\r\n\x1a\n", NULL, 100 }, { NULL, NULL, 0 } };
  static gchar *mime_types[] = { "image/x-nova", NULL };
  static gchar *extensions[] = { "nova", NULL };
  info->name = "nova";
  info->signature = signature;
  info->description = "NOVA image";
