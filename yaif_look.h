/* Tone curves of developed RAWs (looks): output (sRGB-encoded, 0..1) as a function of log2 of the linear
   ...
   darktable is also 7 % more saturated (median of the 10 files, yaif_look_sat) and a bit
   greener (yaif_look_gain); with both, mean error 3.35.
   ponytail: one curve per look for every camera; per-maker curves if other brands look off. */
#define YAIF_LOOK_N 58
static const float yaif_look_x0 = -13.8750f, yaif_look_dx = 0.250000f;
static const float yaif_look[2][YAIF_LOOK_N] = {
  {
...
static unsigned short yaif_look_map(unsigned v, int look) {
  const float *c = yaif_look[look];
  float x = v ? (log2f(v / 65535.0f) - yaif_look_x0) / yaif_look_dx : 0;
  int i = x < 0 ? 0 : x >= YAIF_LOOK_N - 1 ? YAIF_LOOK_N - 2 : (int)x;
...
static const float yaif_look_sat[2] = { 1.0f, 1.07f };
...
static const float yaif_look_gain[2][3] = { { 1.0f, 1.0f, 1.0f }, { 0.98f, 1.0f, 0.97f } };

/* One pixel: 16-bit linear RGB -> 16-bit display RGB (lut: yaif_look_map for this look). */
static void yaif_look_rgb(const unsigned short *lut, const unsigned short *in, unsigned short *out, int look) {
  const float *gn = yaif_look_gain[look];
  float r = lut[(unsigned)(in[0] * gn[0])], g = lut[(unsigned)(in[1] * gn[1])], b = lut[(unsigned)(in[2] * gn[2])], m, k = yaif_look_sat[look];
  if (k != 1.0f) {
    m = (r + g + b) / 3;
